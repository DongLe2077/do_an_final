...
import { Modal, Form, Input, Select, DatePicker, message, Table, List, Card, Badge, Button } from 'antd';
...
export default function HoaDonPage() {
  ...
  const handleThanhToan = async (MaHoaDon) => {
    try {
      await hoaDonService.thanhToan(MaHoaDon);
      message.success('Thanh toán hóa đơn thành công');
      fetchData();
    } catch (error) { message.error(error.response?.data?.message || 'Thanh toán thất bại'); }
  };
  ...
  return (
    <div>
      ...
      <div className="flex-between mb-24">
        ...
        {isAdmin && (
          <div className="feature-card" style={{ maxWidth: 200, padding: 16 }}>
            <div className="feature-card-icon" style={{ width: 36, height: 36, marginBottom: 8 }}>⚡</div>
            <div className="feature-card-badge">QUY TRÌNH THÔNG MINH</div>
            <h3 style={{ fontSize: 16 }}>Tính tiền tự động</h3>
          </div>
        )}
      </div>
                      ...
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                        ...
                        {!isPaid && !isAdmin && (
                          <button
                            ...
                            onClick={() => handleThanhToan(item.MaHoaDon)}
                          >Thanh toán</button>
                        )}
                ...
                <div style={{ padding: '16px', borderBottom: '1px solid #f0f0f0' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Space>
                      ...
                      {!isAdmin && item.TrangThai !== 'Đã thanh toán' && (
                        <Button type="primary" size="small" onClick={() => handleThanhToan(item.MaHoaDon)}>Thanh toán</Button>
                      )}
                    </Space>
                  </div>
                </div>
          ...
          <div className="invoice-summary">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
              <div>
                ...
                <span className={`badge ${firstInvoice.TrangThai === 'Đã thanh toán' ? 'badge-success' : 'badge-danger'}`}>
                  {firstInvoice.TrangThai === 'Đã thanh toán' ? 'ĐÃ THANH TOÁN' : 'CHƯA THANH TOÁN'}
                </span>
              </div>
