...
function ResidentDashboard({ user }) {
  ...
  const handleThanhToan = async (idHoaDon) => {
    try {
      await hoaDonService.thanhToan(idHoaDon);
                    ...
                    <div style={{ textAlign: 'right', display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'flex-end' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        ...
                        {hd.TrangThai === 'Chưa thanh toán' && (
                          <button className="btn btn-primary" onClick={() => handleThanhToan(hd.MaHoaDon)} style={{ padding: '6px 16px', fontSize: 12 }}>
                            Thanh toán
                          </button>
                        )}
                      </div>
...
function AdminDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      const maxRevenue = totalDoanhThu || 1000000;
      setMonthlyData(months.map((label, i) => ({
        label,
        value: Math.floor(Math.random() * maxRevenue * 0.8 + maxRevenue * 0.2),
        active: i === 3
      })));
  ...
  return (
    <div>
      ...
      <div className="greeting-banner">
        <h2>{getGreeting()}</h2>
        <p>Hệ thống ghi nhận {stats.suCoPending} sự cố cần xử lý trong ngày hôm nay.</p>
      </div>
      ...
      <div className="card">
        ...
        <div className="card-subtitle">Tổng hợp 6 tháng gần nhất</div>
        <div className="chart-bars" style={{ height: 180, marginTop: 20 }}>
          {monthlyData.map((item, index) => (
            <div className="chart-bar-item" key={index}>
              ...
              />
              <div className="chart-bar-label" style={item.active ? { color: 'var(--primary)', fontWeight: 700 } : {}}>
                {item.label}
              </div>
            </div>
          ))}
